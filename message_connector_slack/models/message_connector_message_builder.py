...
class MessageConnectorMessageBuilder(models.Model):
    ...
    add_button_action = fields.Boolean(
        ...
        help="Add a button to navigate directly to the " "record_id from the chat",
        states={"active": [("readonly", True)], "canceled": [("readonly", True)]},
        default=True,
    )
    ...
    def _get_button_name(self, record_id):
        ...
        if self.field_button_id:
            field = safe_eval(f"self.{self.field_button_id.name}", {"self": record_id})
        var = [self.prefix_button_name, f"{field}", self.suffix_button_name]
    ...
    def _get_fallback(self, record_id, attachment):
        ...
        if self.add_button_action:
            fallback = f"{self._get_button_name(record_id)} {self._get_url(record_id)}"
