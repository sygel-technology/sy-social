        ...
        for mail_id in self.ids:
            ...
            if mail and mail.model:
                if hasattr(obj, "company_id") and obj.company_id:
                    accepted_model = (
                        mail.model
                        not in obj.company_id.cc_blocked_models.mapped("model")
                    )
                # get company in user
                elif mail.env and mail.env.user and mail.env.user.company_id:
                    accepted_model = (
                        mail.model
                        not in self.env.user.company_id.cc_blocked_models.mapped(
                            "model"
                        )
                    )
                    ...
                    if hasattr(obj, "message_follower_ids"):
                        ...
                        if partners_len > 1:
                            ...
                            # get company in user
                                ...
                                mail.env and mail.env.user and mail.env.user.company_id
                            ):
                                cc_internal = (
                            ...
                            if cc_internal:
                                partners = partners_obj.filtered(
                                    ...
                                    and (
                                        not x.user_ids
                                        or (
                                            x.user_ids.show_in_cc
                                            and group_internal in x.user_ids.groups_id
                                        )
                                        or group_portal in x.user_ids.groups_id
                                    )
                                )
